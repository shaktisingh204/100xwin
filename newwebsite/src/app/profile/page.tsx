...
import { CreditCard, History, FileText, ChevronRight, Users, TrendingUp, Edit2, CheckCircle, XCircle, Loader2, Settings, Gift } from 'lucide-react';
    ...
    if (!editing) {
        return (
            <div className="flex items-center gap-2">
                ...
                >
                    <Edit2 size={13} className="text-white/30 hover:text-[#3BC117] transition-colors" />
                </button>
            </div>
        );
    }
        ...
        <div className="flex flex-col gap-1.5">
            <div className="flex items-center gap-2">
                <input
                    ...
                    className="bg-white/5 border border-white/10 rounded-lg px-3 py-1.5 text-white text-sm outline-none focus:border-[#3BC117]/60 w-40 font-mono"
                />
                {loading ? (
                    <Loader2 size={16} className="animate-spin text-[#3BC117]" />
                ) : (
                    <>
                        <button onClick={handleSave} className="p-1.5 rounded-lg bg-[#3BC117]/10 hover:bg-[#3BC117]/20 transition-colors">
                            <CheckCircle size={14} className="text-[#3BC117]" />
                        </button>
                        <button onClick={() => setEditing(false)} className="p-1.5 rounded-lg hover:bg-white/10 transition-colors">
    ...
    return (
        <>
            <button
                ...
                className="group flex items-center gap-3 bg-[#1a1d21] hover:bg-[#1e2126] border border-amber-500/20 hover:border-amber-500/40 rounded-xl p-4 transition-all text-left w-full"
            >
                <div className="w-10 h-10 rounded-lg bg-amber-500/10 flex items-center justify-center flex-shrink-0">
                    <Gift size={18} className="text-amber-400" />
                </div>
                <div className="flex-1 min-w-0">
                    <h3 className="text-sm font-bold text-white group-hover:text-amber-400 transition-colors">My Bonuses</h3>
                    <p className="text-[11px] text-white/30 truncate">View & manage your active bonuses</p>
                </div>
                <ChevronRight size={16} className="text-white/10 group-hover:text-amber-400/50 transition-colors flex-shrink-0" />
            </button>
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex items-center gap-4">
                <div className="w-14 h-14 rounded-xl bg-[#252830] border border-white/[0.06] flex items-center justify-center text-[#3BC117] font-black text-xl">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    ...
                    {baseMenuItems.map((item) => (
                        <Link
                            key={item.href}
                            href={item.href}
                            className="group flex items-center gap-3 bg-[#1a1d21] hover:bg-[#1e2126] border border-white/[0.06] hover:border-white/10 rounded-xl p-4 transition-all"
                        >
                            <div className={`w-10 h-10 rounded-lg ${item.bg} flex items-center justify-center flex-shrink-0`}>
                                <item.icon size={18} className={item.color} />
                            </div>
                            <div className="flex-1 min-w-0">
                                <h3 className="text-sm font-bold text-white group-hover:text-[#3BC117] transition-colors">{item.label}</h3>
