  ...
  return (
      ...
      controls={
        <>
          ...
          <div className="flex border-b border-white/[0.06]">
            <button
              type="button"
              className="flex-1 py-3 text-sm font-black text-white relative"
            >
              Manual
            ...
            <button
              type="button"
              disabled
              className="flex-1 py-3 text-sm font-black text-white/25 cursor-not-allowed"
            >
              Auto
            </button>
          </div>
          ...
          <div className="flex-1 p-4 space-y-4">
              ...
              <div className="flex bg-white/[0.03] border border-white/[0.06] rounded-lg overflow-hidden">
                {(["fiat", "crypto"] as const).map((w) => (
                  <button
                    ...
                    className={`px-3 py-1.5 font-black uppercase transition-colors ${
                      walletType === w
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="text-[11px] text-white/50 font-black uppercase tracking-wider">
                  Chip Value
                </label>
            ...
            <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-3 space-y-1">
              <div className="text-[10px] uppercase text-white/50 font-black tracking-wider">
                Total Bet
              </div>
            ...
            <div className="grid grid-cols-4 gap-1">
              <button
                ...
                className="flex items-center justify-center py-2 bg-white/[0.03] border border-white/[0.06] rounded text-white/70 hover:text-white disabled:opacity-30"
                title="Undo"
              >
                <Undo2 size={13} />
              </button>
              <button
                ...
                className="py-2 bg-white/[0.03] border border-white/[0.06] rounded text-white/70 text-[11px] font-black hover:text-white disabled:opacity-30"
              >
                ½
              </button>
              <button
    ...
    >
      <div
        ...
        style={{
          background:
            "radial-gradient(ellipse at 50% 30%, #0d1016 0%, #0a0c11 40%, #06080c 100%)",
          minHeight: 360,
        }}
      >
        {/* Status */}
        ...
        {/* Number table */}
        <div className="bg-white/[0.03] border border-white/[0.06] rounded-2xl p-3 space-y-1.5 w-full max-w-3xl">
          <div className="flex items-stretch gap-1 overflow-x-auto">
            ...
            <button
              ...
              disabled={busy}
              className="relative w-10 self-stretch bg-emerald-500 hover:brightness-110 rounded-l-md flex items-center justify-center text-white font-black text-sm flex-shrink-0"
            >
              0
            ...
            <div className="grid grid-rows-3 gap-1 w-10 flex-shrink-0">
              {(["col3", "col2", "col1"] as BetKind[]).map((c) => (
                <button
                  ...
                  onClick={() => placeBet(c)}
                  disabled={busy}
                  className="relative bg-white/[0.06] hover:bg-white/[0.12] rounded text-[10px] text-white/70 font-black h-9"
                >
                  2:1
            ...
            <div className="flex-1 grid grid-cols-3 gap-1 min-w-[440px]">
              {(["dozen1", "dozen2", "dozen3"] as BetKind[]).map((d, i) => (
                <button
                  ...
                  onClick={() => placeBet(d)}
                  disabled={busy}
                  className="relative py-2 bg-white/[0.06] hover:bg-white/[0.12] rounded text-[11px] text-white/70 font-black"
                >
                  {`${i + 1}ST 12`}
...
function ChipBadge({ value }: { value: number }) {
  return (
    <span className="absolute -top-1 -right-1 bg-amber-400 text-[#1a1208] text-[8px] font-black rounded-full px-1 min-w-[14px] h-[14px] flex items-center justify-center shadow">
...
function OutsideBtn({
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      className={`relative py-2 rounded text-[11px] font-black text-white ${
        className || "bg-white/[0.06] hover:bg-white/[0.12]"
      }`}
    >
      {label}
    ...
    <div className="relative" style={{ width: SIZE, height: SIZE }}>
      ...
      >
        {SEQUENCE.map((n, i) => {
          ...
          const fill =
            n === 0 ? "#10b981" : RED.has(n) ? "#dc2626" : "#1a1d24";
          return (
            <g key={i}>
              <path
