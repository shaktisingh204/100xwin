      ...
      <div className="mx-auto max-w-[1820px] px-3 py-3 md:px-5 md:py-5 pb-[calc(var(--mobile-nav-height,80px)+20px)] md:pb-10">
        ...
        {loading ? (
          ...
          <div className="flex min-h-[260px] flex-col items-center justify-center gap-3 rounded-2xl border border-white/[0.06] bg-white/[0.02]">
            ...
            <p className="font-black text-[16px] text-white">{meta.label}</p>
        ...
        {!loading && totalPages > 1 && (
          <div className="mt-6 flex items-center justify-center gap-2">
            <button type="button" onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page <= 1}
              className="flex h-9 w-9 items-center justify-center rounded-xl bg-white/[0.05] border border-white/[0.06] text-white/70 transition hover:bg-white/[0.08] hover:text-white active:scale-90 disabled:opacity-30">
              <ChevronLeft size={16} />
            </button>
            <span className="rounded-xl bg-white/[0.05] border border-white/[0.06] px-4 py-2 text-[13px] font-black text-white">
              {page} / {totalPages}
            </span>
