...
const LIVE_SECTIONS = [
    { title: 'Popular Live',  icon: <PlayCircle size={14} className="text-amber-400"  />, category: 'popular',   sectionKey: 'live' },
    { title: 'Live Roulette', icon: <PlayCircle size={14} className="text-amber-300"  />, category: 'roulette',  sectionKey: 'roulette' },
    { title: 'Live Blackjack',icon: <Dice5      size={14} className="text-amber-400"  />, category: 'blackjack', sectionKey: 'blackjack' },
    { title: 'Live Baccarat', icon: <Coffee     size={14} className="text-amber-300"  />, category: 'baccarat',  sectionKey: 'baccarat' },
    { title: 'Game Shows',    icon: <Tv         size={14} className="text-amber-400"  />, category: 'game_shows',sectionKey: 'shows' },
    { title: 'Live Poker',    icon: <Gamepad2   size={14} className="text-amber-300"  />, category: 'poker',     sectionKey: 'poker' },
];
    ...
    return (
        <div className="flex flex-col bg-[#06080c] min-h-full pb-24">
            ...
            <div className="px-3 mt-3 sticky top-0 z-10 pb-2 bg-[#06080c]">
                <div className={`flex items-center gap-3 bg-white/[0.03] px-4 py-3 rounded-2xl border transition-colors ${searchInput ? 'border-amber-500/40' : 'border-white/[0.06]'}`}>
                    <Search size={16} className={`flex-shrink-0 transition-colors ${searchInput ? 'text-amber-400' : 'text-white/25'}`} />
                    <input
                        type="text"
                        placeholder="Search live games…"
                        value={searchInput}
                        onChange={e => handleSearchChange(e.target.value)}
                        className="flex-1 bg-transparent text-white text-[13px] font-semibold outline-none placeholder:text-white/25"
                    />
                    {searchInput && (
                        <button onClick={clearSearch} className="p-1 rounded-lg bg-white/[0.05] text-white/50">
                            <X size={13} />
                        </button>
                    )}
                </div>
            ...
            {activeProvider && !isSearching && (
                <div className="px-3 mt-2 flex items-center gap-2">
                    <button onClick={() => { setActiveProvider(null); setActiveCat('providers'); }} className="flex items-center gap-1 text-[11px] text-amber-400 font-bold">
                        <Layers size={12} /> Providers <ChevronRight size={11} className="text-white/25" />
                    </button>
            ...
            <div className="mt-4 px-1">
                {isSearching ? (
                    ...
                    <div className="px-2">
                        <p className="text-[10px] font-black uppercase tracking-widest text-white/50 mb-3 px-1">Live Providers</p>
                                ...
                                : providers.map((p, idx) => {
                                    ...
                                    return (
                                        <button key={code || idx} onClick={() => handleProviderClick(code)}
                                            className="flex h-16 items-center justify-center rounded-2xl border border-white/[0.06] bg-white/[0.03] px-3 transition-all active:scale-95 hover:border-amber-500/40">
