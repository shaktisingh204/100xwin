        ...
        <div className="flex-shrink-0 w-[78px] mx-1 group cursor-pointer">
            <div
                className={`relative w-full h-[96px] rounded-xl overflow-hidden shadow-[0_4px_16px_rgba(0,0,0,0.3)] border border-white/[0.06]
                    mb-1.5 group-hover:border-amber-500/40 transition-all group-hover:scale-105 group-hover:shadow-[0_8px_24px_rgba(0,0,0,0.4)]
            ...
            <p className="text-[10px] text-white/50 font-medium truncate text-center leading-tight mt-1">{win.user}</p>
            <p className="text-[11px] font-black text-amber-400 text-center leading-tight">{win.amount}</p>
        </div>
    );
}
...
function TickerSkeleton() {
    return (
        <div className="flex-shrink-0 w-[78px] mx-1">
            <div className="w-full h-[96px] rounded-xl bg-white/[0.04] animate-pulse mb-1.5" />
            <div className="w-2/3 h-2 rounded bg-white/[0.04] animate-pulse mx-auto mb-1" />
            <div className="w-1/2 h-2.5 rounded bg-white/[0.04] animate-pulse mx-auto" />
        </div>
    );
}
...
export default function RecentWinsTicker() {
    ...
    return (
        <div className="w-full bg-white/[0.02] border border-white/[0.04] rounded-2xl py-3">
            {/* Header */}
            <div className="flex items-center gap-2 px-4 mb-3">
                <div className="w-2 h-2 rounded-full bg-amber-500 shadow-[0_0_6px_rgba(245,158,11,0.8)] animate-pulse" />
                <span className="text-sm font-black text-white uppercase tracking-tight">Recent Big Wins</span>
                <TrendingUp size={14} className="text-amber-500 ml-auto" />
            </div>
