...
export default function PromoCardSlider({ onGameLaunch }: PromoCardSliderProps) {
    ...
    if (loading) {
        return (
            <div className="w-full h-[220px] md:h-[340px] bg-white/[0.03] rounded-xl animate-pulse border border-white/[0.06] mb-6" />
        );
    }
    ...
    return (
        <div className="relative w-full mb-6 rounded-xl overflow-hidden group promo-banner-wrap aspect-video sm:aspect-[21/7] border border-white/[0.06]">
            {promoCards.map((c, i) => {
                ...
                return (
                    ...
                    >
                        ...
                        <div className="absolute inset-0 z-10"
                            style={{ background: 'linear-gradient(90deg, rgba(6,8,12,0.88) 0%, rgba(6,8,12,0.55) 55%, rgba(6,8,12,0.1) 100%)' }}
                        />
                        ...
                        <div className="absolute inset-0 z-20 flex flex-col justify-center px-6 md:px-14">
                            {c.tag && (
                                <span className="inline-block py-0.5 px-3 rounded-full bg-amber-500/90 text-[#1a1208] text-[10px] md:text-xs font-black uppercase tracking-widest mb-3 w-fit shadow-[0_0_12px_rgba(245,158,11,0.4)]">
                                    {c.tag}
                                </span>
                            )}

                            {c.subtitle && (
                                <p className="text-xs md:text-sm text-white/70 uppercase tracking-widest font-semibold mb-1 drop-shadow">
                                    {c.subtitle}
                                </p>
                            )}
                            ...
                            {c.description && (
                                <p className="text-xs md:text-sm text-white/70 mb-5 max-w-md line-clamp-2 drop-shadow">
                                    {c.description}
                                </p>
                            )}
                            ...
                            {isCasinoGame(c.buttonLink) ? (
                                <button
                                    ...
                                    className="inline-flex items-center gap-2 bg-gradient-to-r from-amber-500 to-orange-600 hover:brightness-110 text-[#1a1208] text-xs md:text-sm font-black uppercase px-6 py-3 rounded-lg transition-all transform hover:scale-105 shadow-[0_0_16px_rgba(245,158,11,0.5)] w-fit disabled:opacity-70 disabled:cursor-not-allowed disabled:transform-none"
                                >
                                ...
                                <Link
                                    href={c.buttonLink || '/'}
                                    className="inline-flex items-center gap-2 bg-gradient-to-r from-amber-500 to-orange-600 hover:brightness-110 text-[#1a1208] text-xs md:text-sm font-black uppercase px-6 py-3 rounded-lg transition-all transform hover:scale-105 shadow-[0_0_16px_rgba(245,158,11,0.5)] w-fit"
                                >
                                    {c.buttonText || 'Play Now'} <ArrowRight size={15} strokeWidth={3} />
                                </Link>
                            )}
            ...
            {promoCards.length > 1 && (
                <>
                    <button
                        ...
                        aria-label="Previous"
                        className="absolute left-3 md:left-5 top-1/2 -translate-y-1/2 z-30 size-9 md:size-11 bg-black/50 hover:bg-black/80 border border-white/[0.08] text-white rounded-full flex items-center justify-center backdrop-blur-md transition-all opacity-0 group-hover:opacity-100 hover:scale-110"
                    >
                        <ChevronLeft size={20} />
                    </button>
                    <button
                        ...
                        aria-label="Next"
                        className="absolute right-3 md:right-5 top-1/2 -translate-y-1/2 z-30 size-9 md:size-11 bg-black/50 hover:bg-black/80 border border-white/[0.08] text-white rounded-full flex items-center justify-center backdrop-blur-md transition-all opacity-0 group-hover:opacity-100 hover:scale-110"
                    >
