...
function NotifToast({ toast, onDismiss }: { toast: ToastNotif; onDismiss: (id: string) => void }) {
    ...
    return (
        <div
            ...
            className="pointer-events-auto w-[320px] bg-gradient-to-br from-[#12161e] to-[#0c0f14] border border-white/[0.06] rounded-2xl shadow-xl overflow-hidden"
        >
            {/* Glow bar */}
            <div className="h-[2px] w-full bg-gradient-to-r from-amber-500 via-orange-400 to-amber-500 animate-pulse" />
            ...
            <div className="flex items-start gap-3 px-4 py-3">
                ...
                <div className="flex-1 min-w-0">
                    <p className="text-[12px] font-black text-white leading-tight truncate tracking-tight">{toast.title}</p>
                ...
                <button
                    onClick={dismiss}
                    className="flex-shrink-0 mt-0.5 w-5 h-5 flex items-center justify-center rounded-full text-white/25 hover:text-white hover:bg-white/[0.08] transition-colors"
                >
                    <X size={11} />
                </button>
            </div>
        ...
        <>
            ...
            <div className="relative" ref={ref}>
                ...
                <button
                    ...
                    className="relative p-2 rounded-lg text-white/50 hover:text-white hover:bg-white/[0.05] transition-colors"
                >
                    <Bell
                        size={18}
                        style={{
                ...
                {/* Dropdown panel */}
                {open && (
                    <div className="absolute top-full right-0 mt-2 w-80 bg-gradient-to-br from-[#12161e] to-[#0c0f14] border border-white/[0.06] rounded-2xl shadow-xl overflow-hidden z-[200]"
                        style={{ animation: 'dropIn 0.2s cubic-bezier(0.22,1,0.36,1)' }}
                    >
                        {/* Header */}
                        <div className="flex items-center justify-between px-4 py-3 border-b border-white/[0.04]">
                            <h3 className="text-xs font-black text-white uppercase tracking-wider flex items-center gap-1.5">
                                <Bell size={12} className="text-amber-400" /> Notifications
                                {unread > 0 && (
                                    <span className="ml-1 px-1.5 py-0.5 bg-red-500/15 text-red-400 text-[9px] font-bold rounded-full">
                                        {unread} new
                                    </span>
                                )}
                            </h3>
                            {unread > 0 && (
                                <button
                                    onClick={markAllRead}
                                    className="flex items-center gap-1 text-[10px] text-white/25 hover:text-white transition-colors"
                                >
                        ...
                        <div className="max-h-72 overflow-y-auto divide-y divide-white/[0.04]">
                            {notifications.length === 0 ? (
                                ...
                                notifications.map(n => (
                                    <div
                                        key={n._id}
                                        className={`px-4 py-3 transition-colors cursor-pointer group ${n.isRead ? 'opacity-50' : 'bg-amber-500/[0.03] hover:bg-white/[0.02]'}`}
                                        onClick={() => { if (!n.isRead) markRead(n._id); }}
                                    >
                                        <div className="flex items-start gap-2">
                                            {/* Unread dot */}
                                            <span className={`flex-shrink-0 mt-1.5 w-1.5 h-1.5 rounded-full ${n.isRead ? 'bg-transparent' : 'bg-amber-400 shadow-[0_0_4px_rgba(245,158,11,0.6)]'}`} />
                                            <div className="flex-1 min-w-0">
                                                <p className="text-[11px] font-black text-white truncate tracking-tight">{n.title}</p>
                        ...
                        <div className="px-4 py-2.5 border-t border-white/[0.04]">
                            <Link
                                ...
                                className="flex items-center justify-center gap-1 text-[10px] text-white/25 hover:text-amber-400 transition-colors"
                            >
