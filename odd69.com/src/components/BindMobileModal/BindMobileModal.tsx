...
const BindMobileModal: React.FC<BindMobileModalProps> = ({ onClose, onSuccess }) => {
    ...
    const handleVerifyAndBind = async () => {
        ...
        const phoneWithPlus = `+91${phoneNumber.trim()}`;
                    ...
                    <div className="p-6 sm:p-8">
                        {step === 'verify_otp' ? (
                            <div className="text-center animate-in fade-in duration-300">
                                ...
                                <h4 className="text-white font-black text-xl mb-2 tracking-tight">Verify Your Number</h4>
                                <p className="text-white/50 text-sm mb-6">
                                    Enter the 6-digit OTP sent to <br />
                                    <strong className="text-white mt-1 inline-block">+91{phoneNumber}</strong>
                                </p>
                                ...
                                <p className="text-xs text-white/50 mt-2">
                                    Didn't receive it? <button className="text-amber-400 font-black hover:underline" onClick={() => { setStep('form'); setOtpExpiresIn(0); }}>Change number</button>
                                </p>
                            </div>
                        ) : (
                            <div className="animate-in fade-in duration-300">
                                <div className="w-16 h-16 rounded-full bg-amber-500/15 border border-amber-500/30 flex items-center justify-center mx-auto mb-4">
                                    <Smartphone size={30} className="text-amber-400" />
                                </div>
                                <h3 className="text-xl font-black text-white text-center mb-2 tracking-tight">Bind Mobile Number</h3>
                                <p className="text-sm text-white/50 text-center mb-6">
                                    A verified mobile number is required to process withdrawals and secures your account.
                                </p>

                                <div className="flex gap-2 mb-2">
                                    <div className="h-[50px] bg-white/[0.03] border border-white/[0.08] rounded-xl px-4 flex items-center text-white font-medium">
                                        +91
                                    </div>
                                    <input
                                        type="tel"
                                        ...
                                        autoFocus
                                        className={`flex-1 h-[50px] bg-white/[0.03] border rounded-xl px-4 text-white outline-none transition-all focus:ring-[1.5px] placeholder-white/20 font-medium ${error
