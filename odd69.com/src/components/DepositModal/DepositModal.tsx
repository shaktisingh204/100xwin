...
export default function DepositModal({ isOpen, onClose }: DepositModalProps) {
    ...
    const statusMap: Record<string, { label: string; color: string }> = {
        waiting: { label: 'Waiting for payment…', color: 'text-amber-400' },
        confirming: { label: 'Confirming…', color: 'text-amber-400' },
        confirmed: { label: 'Confirmed!', color: 'text-emerald-400' },
        sending: { label: 'Sending funds…', color: 'text-amber-400' },
        finished: { label: 'Complete!', color: 'text-emerald-400' },
        partially_paid: { label: 'Partial — contact support', color: 'text-amber-400' },
        ...
        : promoValidating
            ...
            : promoResult?.valid
                ...
                : promoCode
                    ...
                    : '';
    const lightFieldClass = 'rounded-[18px] border border-white/[0.08] bg-white/[0.03] px-4 py-3';
    const lightCardClass = 'rounded-[20px] border border-white/[0.08] bg-gradient-to-br from-[#12161e] to-[#0c0f14] shadow-lg';
    ...
    const renderBodyTabs = () => (
        <div className="grid grid-cols-2 gap-1 rounded-[18px] border border-white/[0.08] bg-white/[0.03] p-1">
            <button
                onClick={() => {
                    setActiveTab('fiat');
                    resetCryptoState();
                ...
                className={`flex items-center justify-center gap-2 rounded-[14px] px-4 py-3 text-sm font-semibold transition-all ${activeTab === 'fiat'
                    ? 'border border-amber-500/30 bg-gradient-to-br from-[#12161e] to-[#0c0f14] text-white shadow-lg'
            ...
            >
                <span className="flex h-5 w-5 items-center justify-center rounded-full bg-gradient-to-br from-amber-500 to-orange-600 text-[10px] font-bold text-[#1a1208]">
                    B
                </span>
                {cryptoTabLabel}
            </button>
    ...
    const renderConfigureView = () => {
        ...
        return (
            <div className="flex min-h-0 flex-1 flex-col bg-gradient-to-br from-[#12161e] to-[#0c0f14]">
                <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4 sm:px-5 sm:py-5">
                    <div className="mx-auto w-full max-w-md space-y-4 sm:max-w-lg">
                        {canShowFiatTab && renderBodyTabs()}

                        {!canShowFiatTab && (
                            <div className="rounded-[18px] border border-white/[0.08] bg-white/[0.03] px-4 py-3 text-center text-sm font-semibold text-white">
                                {cryptoTabLabel}
                            </div>
                        )}
                            ...
                            showFiatAmountStep ? (
                                <>
                                    ...
                                    {!isForcedManualFlow && selectedGateway && (
                                        <div className="px-1">
                                            <div className="flex items-center gap-3">
                                                {renderGatewayLogo(selectedGateway, 'flex h-14 w-14 items-center justify-center overflow-hidden text-white', 'text-2xl')}
                                                <div>
                                                    <p className="text-2xl font-black tracking-tight text-white">
                                                        {selectedGateway.label}
                                                    </p>
                                                    <p className="mt-1 text-sm text-white/50">{selectedGateway.sub}</p>
                                                </div>
                                            </div>
                                        </div>
                                    )}
                                    ...
                                    {isForcedManualFlow && gatewayRetryState && (
                                        <div className="rounded-[20px] border border-amber-500/30 bg-amber-500/10 px-4 py-3">
                                            <p className="text-sm font-semibold text-white">{gatewayRetryState.message}</p>
                                    ...
                                    <div className={lightFieldClass}>
                                        ...
                                        <input
                                            ...
                                            className="mt-1 w-full bg-transparent text-[2rem] font-black tracking-tight text-white placeholder-white/20 focus:outline-none"
                                        />
                                    </div>
                                        ...
                                        {currentQuickAmounts.map((value) => (
                                            <button
                                                ...
                                                className={`rounded-full px-4 py-2 text-sm font-semibold transition-all ${amount === value
                                                    ? 'bg-gradient-to-br from-amber-500 to-orange-600 text-[#1a1208] shadow-lg'
                                ...
                                <>
                                    <div className={`${lightFieldClass} flex items-center justify-between`}>
                                        <div className="flex items-center gap-3">
                                            <span className="flex h-6 w-6 items-center justify-center rounded-full bg-amber-500/15 text-[11px] font-bold text-amber-400">
                                                {fiatSymbol}
                                            </span>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        {gatewayOptions.map((gateway) => (
                                            <button
                                                ...
                                                className="rounded-[20px] border border-white/[0.08] bg-white/[0.03] p-4 text-left transition-all hover:-translate-y-0.5 hover:bg-white/[0.06] hover:border-amber-500/30"
                                            >
                                                {renderGatewayLogo(
                                                    gateway,
                                                    'flex h-12 w-full items-center overflow-hidden text-white',
                                                    'text-2xl',
                                                    'h-full w-full object-contain object-left py-1',
                                                )}
                                                <p className="mt-2 text-base font-semibold text-white">{gateway.label}</p>
                                                <p className="mt-1 text-sm text-white/50">{gateway.sub || `${fiatSymbol}${formatAmount(gateway.minDeposit)} min`}</p>
                                            </button>
                                        ))}
                                    </div>
                                </>
                            ...
                            <>
                                <div className="relative">
                                    ...
                                    >
                                        ...
                                        <ChevronDown className={`h-5 w-5 text-white/50 transition-transform ${isCoinDropdownOpen ? 'rotate-180' : ''}`} />
                                    </button>

                                    {isCoinDropdownOpen && (
                                        <div className={`relative mt-2 z-10 ${lightCardClass}`}>
                                            <div className="p-4">
                                                <div className="flex items-center gap-2 rounded-[16px] border border-white/[0.08] bg-white/[0.03] px-3 py-3 focus-within:border-amber-500/60">
                                            ...
                                            <div className="max-h-72 overflow-y-auto px-3 pb-3">
                                                {displayedCoinOptions.map((coin) => (
                                                    <button
                                                        ...
                                                        className="flex w-full items-center gap-3 border-b border-white/[0.06] px-2 py-3 text-left last:border-b-0 hover:bg-white/[0.04]"
                                                    >
                                                        <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-amber-500/15 text-[11px] font-bold text-amber-400">
                                                            {buildCoinMonogram(coin.label, coin.code).slice(0, 1)}
                                                        </span>
                                                        <div className="min-w-0 flex-1">
                                                            <p className="truncate text-base font-medium text-white">{coin.label}</p>
                                                            <p className="mt-0.5 text-sm text-white/50">{coin.code}</p>
                                                        </div>
                                                    </button>
                                                ))}
                                            </div>
                                ...
                                {selectedCoin && (
                                    <div className="relative">
                                        ...
                                        >
                                            ...
                                            <ChevronDown className={`h-5 w-5 text-white/50 transition-transform ${isNetworkDropdownOpen ? 'rotate-180' : ''}`} />
                                        </button>
                                                ...
                                                <div className="max-h-72 overflow-y-auto px-3 py-2">
                                                    {displayedNetworkOptions.map((coin) => (
                                                        <button
                                                            ...
                                                            className="flex w-full items-start justify-between gap-3 border-b border-white/[0.06] px-2 py-3 text-left last:border-b-0 hover:bg-white/[0.04]"
                                                        >
                                                            <div>
                                                                <p className="text-base font-medium text-white">{coin.network}</p>
                                ...
                                {selectedNetworkOption && (
                                    <>
                                        ...
                                        <p className="text-center text-sm leading-relaxed text-white/70">
                                            Minimum <span className="font-semibold text-white">USD {formatAmount(minDepositCrypto)}</span>
                                            <br />
                            ...
                            <div className="space-y-3">
                                {promoCode && (
                                    <div className={`rounded-[18px] border px-4 py-3 text-sm ${promoBlockingError
                                        ...
                                        : 'border-amber-500/30 bg-amber-500/10 text-white/70'
                                        }`}>
                                        <div className="flex items-start justify-between gap-3">
                                            <div>
                                                <p className="font-semibold text-white">{appliedBonusTitle}</p>
                                            ...
                                            <div className="flex items-center gap-2">
                                                ...
                                                <button
                                                    ...
                                                    type="button"
                                                    className="rounded hover:bg-white/[0.08] p-1 text-white/50 hover:text-white transition-colors"
                                                    title="Remove Bonus"
                                                >
                                                    <X className="h-4 w-4" />
                                                </button>
                ...
                {showFooterAction && (
                    <div className="shrink-0 border-t border-white/[0.06] bg-gradient-to-br from-[#12161e] to-[#0c0f14] px-4 py-4 sm:px-5">
                        <div className="mx-auto flex max-w-md flex-col gap-3 sm:max-w-lg">
                            {!isForcedManualFlow && (
                                <p className={`text-sm ${promoBlockingError || currentAmountError || selectedMethodError ? 'text-red-400' : 'text-white/50'}`}>
                                    {currentSubmitHint}
                                </p>
                            )}
                            ...
                            <button
                                ...
                                className="inline-flex w-full items-center justify-center gap-2 rounded-[16px] bg-gradient-to-br from-amber-500 to-orange-600 px-6 py-3.5 text-sm font-semibold text-[#1a1208] shadow-lg transition-all hover:brightness-110 disabled:cursor-not-allowed disabled:opacity-60"
                            >
                                {loading ? (
                                    <>
    ...
    const renderIframeView = () => (
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden bg-gradient-to-br from-[#12161e] to-[#0c0f14]">
            <div className="shrink-0 flex items-center justify-end border-b border-white/[0.06] bg-gradient-to-br from-[#12161e] to-[#0c0f14] px-4 py-3 sm:px-5">
                <button
                    onClick={onClose}
                    className="inline-flex items-center justify-center rounded-full border border-white/[0.08] bg-white/[0.03] px-6 py-2 text-sm font-semibold text-white/70 transition-colors hover:bg-white/[0.06] hover:text-white"
                >
                    Close
                </button>
            </div>
            <div className="min-h-0 flex-1 overflow-hidden bg-gradient-to-br from-[#12161e] to-[#0c0f14]">
                <iframe
                ...
                <div className="mx-auto w-full max-w-md space-y-4 sm:max-w-lg">
                    ...
                    <div className={`${lightFieldClass} flex items-center justify-between`}>
                        <div className="flex items-center gap-3">
                            ...
                            <span className="text-base font-medium text-white">{selectedCoin?.label || paymentData?.payCurrency}</span>
                        </div>
                        <ChevronDown className="h-5 w-5 text-white/50" />
                    </div>

                    <div className={`${lightFieldClass} flex items-center justify-between`}>
                        <span className="text-base font-medium text-white">{selectedNetworkOption?.network}</span>
                        <ChevronDown className="h-5 w-5 text-white/50" />
                    </div>
                    ...
                    {paymentData && (
                        <div className="rounded-[22px] border border-white/[0.08] bg-white/[0.03] p-4 shadow-lg">
                            ...
                            {paymentData.payAmount > 0 && (
                                <div className="mb-4 rounded-[16px] border border-amber-500/30 bg-amber-500/10 px-4 py-3">
                                    <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-amber-400">
                                        Send exactly
                                    </p>
                                    <div className="mt-1 flex items-center justify-between gap-3">
                                        <p className="break-all text-lg font-black tracking-tight text-white sm:text-xl">
                                        ...
                                        <button
                                            ...
                                            className="shrink-0 inline-flex items-center gap-1 rounded-[10px] border border-amber-500/40 bg-amber-500/10 px-2.5 py-1.5 text-[11px] font-semibold text-amber-400 transition-colors hover:bg-amber-500/20"
                                        >
                            ...
                            <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                                <div className="flex justify-center sm:block">
                                    <div className="inline-flex items-center justify-center rounded-[16px] border border-white/[0.08] bg-white/[0.03] p-4">
                                        {/* QR rendering placeholder: use an external QR image service so the
                                            deposit address is still scannable without a QR library. */}
                                        {/* eslint-disable-next-line @next/next/no-img-element */}
                                        <img
                                            src={`https://api.qrserver.com/v1/create-qr-code/?size=140x140&bgcolor=ffffff&color=07111b&data=${encodeURIComponent(paymentData.payAddress)}`}
                                            alt="Deposit address QR"
                                            width={140}
                                            height={140}
                                            className="block h-[140px] w-[140px] rounded-[8px] bg-white"
                                            onError={(event) => {
                                                // Fallback: hide the broken image and rely on the address below
                                                event.currentTarget.style.display = 'none';
                                            }}
                                        />
                                        <QrCode className="hidden h-24 w-24 text-[#07111b]" aria-hidden />
                                    </div>
                                </div>

                                <div className="min-w-0 flex-1 text-center sm:text-left">
                                    <p className="text-[1.75rem] font-black tracking-tight text-white">Deposit address</p>
                                    ...
                                    <button
                                        ...
                                        className="mt-4 inline-flex items-center justify-center gap-2 rounded-[12px] bg-gradient-to-br from-amber-500 to-orange-600 px-5 py-2.5 text-sm font-semibold text-[#1a1208] shadow-lg transition-colors hover:brightness-110"
                                    >
                    ...
                    <div className="flex items-center justify-between rounded-[18px] border border-white/[0.08] bg-white/[0.03] px-4 py-3 text-sm text-white/50">
    ...
    const renderSuccessView = () => (
        <div className="flex flex-1 items-center justify-center bg-gradient-to-br from-[#12161e] to-[#0c0f14] px-4 py-8 sm:px-6">
            <div className="w-full max-w-md rounded-[28px] border border-white/[0.08] bg-white/[0.03] p-6 text-center shadow-lg">
                ...
                <p className="mt-5 text-[10px] font-semibold uppercase tracking-[0.24em] text-amber-400">Transfer Complete</p>
                <h3 className="mt-3 text-3xl font-black tracking-tight text-white">Deposit Confirmed</h3>
                <p className="mx-auto mt-3 max-w-sm text-sm leading-relaxed text-white/70">
                    Your crypto deposit of <span className="font-semibold text-white">${amount} USD</span> has been confirmed and credited to your balance.
                </p>

                <div className="mt-6 grid gap-2 sm:grid-cols-2">
                    <div className="rounded-[18px] border border-white/[0.08] bg-white/[0.03] p-3">
                        <p className="text-[10px] uppercase tracking-[0.2em] text-white/50">Amount</p>
                        <p className="mt-2 text-sm font-semibold text-white">${amount}</p>
                    </div>
                    <div className="rounded-[18px] border border-white/[0.08] bg-white/[0.03] p-3">
                        <p className="text-[10px] uppercase tracking-[0.2em] text-white/50">Route</p>
                        <p className="mt-2 text-sm font-semibold text-white">{selectedNetworkOption?.network || selectedCoin?.label || 'Crypto'}</p>
                    </div>
                </div>

                <button
                    onClick={() => { resetCryptoState(); onClose(); }}
                    className="mt-7 inline-flex items-center justify-center rounded-[16px] bg-gradient-to-br from-amber-500 to-orange-600 px-8 py-3 text-sm font-semibold text-[#1a1208] shadow-lg transition-all hover:brightness-110"
                >
                    Done
                </button>
            </div>
    ...
    return (
        <>
            ...
            <div className="pointer-events-none fixed inset-0 z-50 flex items-end justify-center p-0 sm:items-center sm:p-5">
                <div className="pointer-events-auto relative flex h-[100dvh] max-h-[100dvh] w-full flex-col overflow-hidden overscroll-contain bg-gradient-to-br from-[#12161e] to-[#0c0f14] text-white shadow-2xl sm:h-[85vh] sm:min-h-[85vh] sm:max-h-[85vh] sm:max-w-[430px] sm:rounded-2xl sm:border sm:border-white/[0.06]">
                    <div className="relative flex min-h-0 flex-1 flex-col">
                        <div className="shrink-0 border-b border-white/[0.06] bg-gradient-to-br from-[#12161e] to-[#0c0f14] px-5 pb-4 pt-5">
                            <div className="flex items-center justify-between">
                                ...
                                <button
                                    onClick={onClose}
                                    className="flex h-9 w-9 items-center justify-center rounded-full text-white/50 transition-colors hover:bg-white/[0.06] hover:text-white"
                                >
                                    <X className="h-5 w-5" />
                                </button>
                            </div>
                        ...
                        {!isIndia && (
                            <div className="shrink-0 border-b border-amber-500/20 bg-amber-500/10 px-4 py-3 text-sm text-white/70">
