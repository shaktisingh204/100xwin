 ...
 * Game-specific controls are passed in via `children` (rendered between the
 * amount block and the action button).
 *
    ...
    <>
      ...
      <div className="flex border-b border-white/[0.06]">
        ...
        <button
          type="button"
          disabled
          className="flex-1 py-3 text-sm font-bold text-white/25 cursor-not-allowed"
      ...
      <div className="flex-1 p-4 space-y-4">
        ...
        <div>
          ...
          <div className="flex items-center gap-1.5">
            <div className="flex-1 relative flex items-center bg-white/[0.03] border border-white/[0.08] rounded-lg overflow-hidden focus-within:border-amber-500/40">
              <div className="pl-3 pr-1 text-sm font-bold text-white/70">
                {sym}
              </div>
              <input
                type="number"
              ...
              <div className="flex flex-col border-l border-white/[0.06]">
                <button
                  ...
                  disabled={locked}
                  className="px-2 py-1 hover:bg-white/[0.05] text-white/50 hover:text-white transition-colors disabled:opacity-40"
                >
                  <ChevronUp size={12} />
                </button>
                <button
                  ...
                  disabled={locked}
                  className="px-2 py-1 hover:bg-white/[0.05] text-white/50 hover:text-white transition-colors disabled:opacity-40 border-t border-white/[0.06]"
                >
                  <ChevronDown size={12} />
                </button>
              </div>
            ...
            <button
              ...
              onClick={() => adjust("half")}
              disabled={locked}
              className="px-3 py-2.5 bg-white/[0.03] border border-white/[0.08] rounded-lg text-xs font-bold text-white/70 hover:text-white hover:border-white/[0.12] transition-all disabled:opacity-40"
            >
              ½
            </button>
            <button
          ...
          <div className="grid grid-cols-4 gap-1 mt-2">
            {QUICK_BETS.map((n) => (
              <button
                ...
                disabled={locked}
                className="py-1.5 bg-white/[0.03] border border-white/[0.08] hover:border-white/[0.12] hover:text-white rounded-lg text-white/70 text-xs font-bold transition-all disabled:opacity-40"
              >
                {fmtBet(n)}
              </button>
            ))}
