...
export class SportsService implements OnModuleInit {
    ...
    private getRedis(): Redis {
        if (!this.redisClient) {
            ...
            this.redisClient.on('error', (e) => this.logger.warn(`Redis error: ${e.message}`));
        }
    ...
    constructor(
        ...
        @InjectModel(HomeEvent.name) private homeEventModel: Model<HomeEventDocument>,
    ...
    /** Return all team icons (cached 60s in Redis) */
    async getTeamIcons() {
        ...
        try {
            ...
            await redis.set(CACHE_KEY, JSON.stringify(result), 'EX', 60).catch(() => {});
            return result;
        } catch (e) {
    ...
    private async makeApiRequest<T>(url: string, payload: any): Promise<{ data: T; status: number }> {
        ...
        const requestTask = this.requestQueue.then(async () => {
            ...
            if (timeSinceLast < this.REQUEST_INTERVAL_MS) {
                const waitTime = this.REQUEST_INTERVAL_MS - timeSinceLast;
                if (waitTime > 0) {
                    this.logger.debug(`Rate limit: Waiting ${waitTime}ms before calling ${url}`);
                    await new Promise(resolve => setTimeout(resolve, waitTime));
                }
            }
        ...
        const updateLiveStatuses = async () => {
            try {
                ...
                const result = await this.eventModel.updateMany(
                    { match_status: { $nin: ['In Play', 'Completed'] }, open_date: { $lte: now.toISOString() } },
                    { $set: { match_status: 'In Play' } }
                );
                if (result.modifiedCount > 0) {
                    this.logger.log(`[LiveStatusJob] Marked ${result.modifiedCount} events as 'In Play'`);
                }
            } catch (e) {
    ...
    async cleanupOldCompletedMatches() {
        try {
            ...
            const staleEvents = await this.eventModel.find({
            ...
            const affectedCompetitionIds = [...new Set(staleEvents.map(e => e.competition_id))];

            this.logger.log(`[MatchCleanup] Found ${eventIds.length} stale matches to remove.`);
            ...
            const eventsRes = await this.eventModel.deleteMany({ event_id: { $in: eventIds } });
            ...
            for (const compId of affectedCompetitionIds) {
                const count = await this.eventModel.countDocuments({ competition_id: compId });
            ...
            this.logger.log(`[MatchCleanup] Updated market_count for ${affectedCompetitionIds.length} competitions.`);
        } catch (error) {
    ...
    async syncAll() {
        this.logger.log('[syncAll] Sync is handled by sports-sync-rust. Skipping NestJS sync.');
        return { message: 'Sync is handled by the Rust service (sports-sync-rust). Restart it to trigger a sync.' };
    }
    ...
    private readonly DIAMOND_API_URL = process.env.DIAMOND_API_URL || "https://diamond-sports-api-d247-sky-exchange-betfair.p.rapidapi.com";
    private readonly DIAMOND_API_HOST = process.env.DIAMOND_API_HOST || "diamond-sports-api-d247-sky-exchange-betfair.p.rapidapi.com";
    private readonly DIAMOND_API_KEY = process.env.DIAMOND_API_KEY || "fdaa78ee08mshb866b2d82236d8cp18a100jsn764d59a3c2a1";
        ...
        try {
            ...
            for (const match of response.data) {
                try {
                    ...
                    const existingHeader = await this.eventModel.findOne({ event_id: match.match_id });
                    if (existingHeader) {
                    ...
                    this.logger.error(`Failed to process match ${match.match_id}: ${err.message}`, err.stack);
                }
            }
    ...
    public async importMarkets() {
        ...
        for (const match of matches) {
            try {
                ...
                if (matchId) {
                    await this.importExchangeMarkets(accessToken, sportId, tournamentId, matchId, false);
                    await this.importSessionMarkets(accessToken, sportId, tournamentId, matchId, false);
                } else {
                    this.logger.warn(`Skipping event with no match_id (internal id: ${match._id})`);
                }

                await new Promise(resolve => setTimeout(resolve, 100));

            } catch (error) {
                this.logger.error(`Error processing match ${match.event_id} in importMarkets: ${error.message}`);
            }
        }

        const activeCount = await this.marketModel.countDocuments({ is_active: true });
        this.logger.log(`Market import check completed. Total Active Markets in DB: ${activeCount}`);
        return { message: 'Market import triggered for all matches', activeMarkets: activeCount };
    }
    ...
    public async importExchangeMarkets(token: string | undefined, sportId: string, tournamentId: string, matchId: string, setActive: boolean = true) {
        // Cooldown Check
    ...
    public async importSessionMarkets(token: string | undefined, sportId: string, tournamentId: string, matchId: string, setActive: boolean = true) {
        // Cooldown Check
    ...
    async getEvents(sportId: number) {
        ...
        const competitions = await this.competitionModel.find({ sport_id: sportId.toString() }, { competition_id: 1 }).lean();
        ...
        try { await redis.set(cacheKey, JSON.stringify(events), 'EX', 30); } catch (e) { /* non-fatal */ }
        return events;
    }
    ...
    async getLiveEvents(sportId?: number) {
        ...
        try {
            const cached = await redis.get(cacheKey);
            if (cached) return JSON.parse(cached);
        } catch (e) { /* Redis miss — fall through to DB */ }
        ...
        const events = await this.eventModel.aggregate([
            ...
            // Derive sport_id from competition when not stored directly on event
            {
                $addFields: {
                    ...
                    'competition.sport': { sport_id: { $ifNull: ['$sport_id', '$competition.sport_id'] } },
                    competition_name: { $ifNull: ['$competition_name', '$competition.competition_name', ''] }
                }
            },
            // Only return fields needed for listing cards — no markets
            {
                $project: {
                    event_id: 1, event_name: 1, match_status: 1, open_date: 1,
                    home_team: 1, away_team: 1, score1: 1, score2: 1,
                    competition_id: 1, competition_name: 1, sport_id: 1,
                    in_play: 1, competition: 1
                }
            }
        ]);

        try { await redis.set(cacheKey, JSON.stringify(events), 'EX', 10); } catch (e) { /* non-fatal */ }
    ...
    async getUpcomingEvents(sportId?: number) {
        ...
        let matchQuery: any = { match_status: { $in: ['Pending', null] }, isVisible: { $ne: false } };
        // Note: sport filtering is done on the frontend since older events may lack sport_id
     ...
     * feed field names to frontend format, and returns a unified list.
     * This bypasses MongoDB aggregation entirely for speed and reliability.
     */
    async getAllEvents(sportId?: number) {
        const cacheKey = `sports:all:v3`; // v3 = includes market + selection ids for sports-card betting
        const redis = this.getRedis();
        ...
        await Promise.all(activeSports.map(async (sport) => {
            try {
                ...
                for (const e of feedEvents) {
                    ...
                    try {
                        ...
                        if (oddsRaw) {
                            ...
                            const mkt = markets.find(m =>
                                String(m?.mname ?? '').toLowerCase().includes('match odds')
                            ) || markets[0];

                            if (mkt) {
                                // rt = array of {ri, ib, rt, bv, nat}
                                const rt: any[] = mkt.rt || [];
                                const marketId = String(mkt.mid || mkt.market_id || mkt.id || '');
                                const marketName = String(mkt.mname || mkt.market_name || 'Match Odds');
                                ...
                                if (grouped.size === 0 && mkt.section) {
                                    for (const sel of (mkt.section || [])) {
                                        ...
                                        const back = odds.find((o: any) => o.otype === 'back')?.odds ?? null;
                                        const selectionId = String(sel.sid ?? sel.selectionId ?? sel.selection_id ?? name);
                                        if (name) {
                    ...
                    } catch (_) { /* non-fatal — show card without odds */ }
        ...
        const deduped = [...new Map(allEvents.map(e => [e.event_id, e])).values()];

        // Short TTL (3s) so odds stay reasonably fresh without hammering Redis on every request
        try { await redis.set(cacheKey, JSON.stringify(deduped), 'EX', 3); } catch (e) { /* non-fatal */ }
    ...
    async placeBet(
        ...
        for (const runner of market.runners_data || []) {
            ...
            if (runner.nat === selectionName || (runner.id && runner.id.toString() === selectionId?.toString())) {
                const oddsArray = runner.odds || [];
                // Check if requested rate still exists as a valid active odd
                for (const odd of oddsArray) {
                    if (odd.otype?.toLowerCase() === type?.toLowerCase() && parseFloat(odd.odds) === parseFloat(rate.toString())) {
                        oddsMatch = true;
                        break;
                    }
                }
        ...
        if (!oddsMatch) {
            throw new Error(`Price changed. The requested odds ${rate} for ${type?.toUpperCase()} are no longer available.`);
        }
    ...
    public async syncSidebar() {
        ...
        try {
            const response = await firstValueFrom(
                this.httpService.get<DiamondSidebarResponse>(`${this.DIAMOND_API_URL}/sports/tree`, {
                    headers: {
    ...
    public async getMatchDetailsData(sportId: string, matchId: string, userId?: number) {
        ...
        try {
            ...
            if (resData && resData.success && resData.data && resData.data.length > 0) {
                ...
                if (blockedTypes.includes('ODDS')) {
                    finalData = finalData.filter((m: any) => m.gtype !== 'match' && m.gtype !== 'MATCH');
                }
                if (blockedTypes.includes('FANCY')) {
                    finalData = finalData.filter((m: any) => m.gtype !== 'fancy' && m.gtype !== 'FANCY' && m.gtype !== 'session');
                }
            ...
            this.logger.error(`Error fetching match details for GMID ${matchId}: ${error.message}`);
            return null;
        }
    }
     ...
     */
    public async checkOdds(items: { eventId?: string; marketId: string; selectionId: string; odds: number }[]) {
        type OddsResult = {
            marketId: string;
            selectionId: string;
        ...
        for (const [eventId, group] of eventGroups) {
            ...
            try {
                ...
                this.logger.warn(`checkOdds: Redis miss for ${eventId}: ${redisErr.message}`);
            }

            // ── 2. Live Sports API if Redis missed ───────────────────────────────
            if (!marketsData) {
                try {
                    // Look up sport_id from events collection (required by the API as sportsid)
                    let sportId = '4'; // fallback (cricket)
                    try {
                        const event = await this.eventModel.findOne({ event_id: eventId }).lean() as any;
                    ...
                    if (oddsMap && oddsMap[eventId]) {
                        ...
                        this.logger.debug(`checkOdds: Live API hit for event ${eventId} (sport ${sportId})`);
                    }
                } catch (apiErr) {
                    this.logger.warn(`checkOdds: Live API failed for ${eventId}: ${apiErr.message}`);
                }
            }
            ...
            for (const item of group) {
                ...
                if (marketsData && marketsData.length > 0) {
                    // Find the matching market by mid (no fallback — wrong market = no data)
                    const market = marketsData.find((m: any) => {
                        const mid = String(m.mid || '');
                        return mid === item.marketId || item.marketId.endsWith(`_${mid}`) || item.marketId === mid;
                    });
                    ...
                    if (market) {
                        ...
                        // Debug: show exactly what sids are available vs what we're looking for
                        ...
                        for (const runner of section) {
                            const sid = String(runner.sid || runner.selectionId || runner.selection_id || '');
                            if (sid === String(item.selectionId)) {
                                const oddsArr: any[] = runner.odds || [];
                                const backOdd = oddsArr.find((o: any) => o.otype === 'back' || o.ib === true);
                                if (backOdd) {
                                    currentOdds = parseFloat(String(backOdd.odds || backOdd.rt || backOdd.price || 0));
                                }
                                break;
                            }
                        }
                ...
                if (currentOdds === null && !suspended) {
                    try {
                        const dbMarket = await this.marketModel.findOne({ market_id: item.marketId }).lean() as any;
                        if (dbMarket) {
                            if (dbMarket.is_active === false || dbMarket.status === 'SUSPENDED') suspended = true;
                            const section: any[] = dbMarket.runners_data || dbMarket.marketOdds || [];
                            for (const runner of section) {
                                const sid = String(runner.sid || runner.selectionId || runner.selection_id || '');
                                if (sid === String(item.selectionId)) {
                                    const oddsArr: any[] = runner.odds || [];
                ...
                const oddsActuallyChanged = currentOdds !== null && Math.abs(currentOdds - item.odds) > 0.01;
    ...
    public async getMatchWithMarketsFromDB(matchId: string) {
        try {
            ...
            const [markets, competition] = await Promise.all([
                ...
                this.competitionModel.findOne({ competition_id: event.competition_id }).lean()
            ]);
    ...
    public async proxyStream(targetUrl: string): Promise<{ content: string | Buffer; contentType: string } | null> {
        ...
        try {
            ...
            this.logger.log(`[proxyStream] ← HTTP ${response.status} content-type=${response.headers['content-type']}`);
            ...
            if (contentType.includes('text/html')) {
                ...
                } catch (_) { /* URL parse failed, skip base injection */ }
    ...
    public async getTvUrl(sportId: string, matchId: string): Promise<string | null> {

        const url = `${this.SPORTS_BASE_URL}/api/v1/sports/tv?gmid=${matchId}&sportsid=${sportId}`;
        this.logger.log(
            `[getTvUrl] curl -X GET "${url}" -H "x-turnkeyxgaming-key: ${this.SPORTS_API_KEY}"`
        );
        try {
            const response = await firstValueFrom(
                this.httpService.get(url, {
            ...
            this.logger.log(`[getTvUrl] Response (${response.status}): ${JSON.stringify(response.data)}`);
            // API response: { success: true, data: { tv_one: "..." } }
            return response.data?.data?.tv_one || null;
        } catch (error) {
            const status = error?.response?.status;
            const body = JSON.stringify(error?.response?.data ?? {});
            this.logger.warn(`[getTvUrl] FAILED (${status}) match=${matchId}: ${error.message} | body=${body}`);
            return null;
        }
    }

    public async getScoreUrl(sportId: string, matchId: string): Promise<string | null> {
        const url = `${this.SPORTS_BASE_URL}/api/v1/sports/score?gmid=${matchId}&sportsid=${sportId}`;
        this.logger.log(
            `[getScoreUrl] curl -X GET "${url}" -H "x-turnkeyxgaming-key: ${this.SPORTS_API_KEY}"`
        );
        try {
            const response = await firstValueFrom(
                this.httpService.get(url, {
            ...
            this.logger.log(`[getScoreUrl] Response (${response.status}): ${JSON.stringify(response.data)}`);
            // API response: { success: true, data: { scoreurl: "..." } }
            return response.data?.data?.scoreurl || null;
        } catch (error) {
            const status = error?.response?.status;
            const body = JSON.stringify(error?.response?.data ?? {});
            // 404 = "Score room not found" — expected when match not live yet
            if (status === 404) {
                this.logger.log(`[getScoreUrl] 404 (score room not found) match=${matchId}`);
            } else {
                this.logger.warn(`[getScoreUrl] FAILED (${status}) match=${matchId}: ${error.message} | body=${body}`);
            }
            return null;
        }
    }
    ...
    public async getScorecardAndTvData(sportId: string, matchId: string) {
        ...
        try {
            ...
            this.logger.error(`Error fetching TV/Scorecard data for event ${matchId}: ${error.message}`);
            return null;
        }
    }
    ...
    public async getTopEvents() {
        ...
        try {
            ...
            try { await redis.set(cacheKey, JSON.stringify(data), 'EX', 30); } catch (e) { /* non-fatal */ }
            return data;
        } catch (error) {
    ...
    public async toggleEventPopular(eventId: string, isPopular: boolean, eventName?: string, sportId?: number) {
        try {
            if (isPopular) {
                ...
                let sid = sportId;
                if (!name || !sid) {
                    const event = await this.eventModel.findOne({ event_id: eventId }).lean();
                    if (event) {
                ...
                await this.topEventModel.updateOne(
                    ...
                    { $set: { event_id: String(eventId), name: name || 'Unknown', sportId: sid || 0, lid: '' } },
                    { upsert: true }
                );
            ...
            this.logger.error(`Error toggling popular for event ${eventId}: ${error.message}`);
            throw error;
        }
    }
}
