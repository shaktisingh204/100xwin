...
import { PROVIDERS_MAP, LKR_PROVIDERS, INR_PROVIDERS } from '../config/casino.config';
...
import { CasinoCategory, CasinoCategoryDocument } from './schemas/casino-category.schema';
import { CasinoSectionGame, CasinoSectionGameDocument } from './schemas/casino-section-game.schema';
...
export class CasinoService {
    ...
    private readonly CASINO_AUTH_URL = 'https://auth.worldcasinoonline.com/api/auth';
    ...
    private readonly HUIDU_AGENCY_UID = process.env.HUIDU_AGENCY_UID || 'ab72cfab44395f7063c6f0c0f05b2325';
    private readonly HUIDU_AES_KEY = process.env.HUIDU_AES_KEY || 'cf847d09b90ae11051a5f09769a96578';
    private readonly HUIDU_PLAYER_PREFIX = process.env.HUIDU_PLAYER_PREFIX || 'h9f5c4';
    private readonly HUIDU_BASE_URL = process.env.HUIDU_BASE_URL || 'https://jsgame.live';
    ...
    constructor(
        ...
        @InjectModel(CasinoCategory.name) private casinoCategoryModel: Model<CasinoCategoryDocument>,
        @InjectModel(CasinoSectionGame.name) private casinoSectionGameModel: Model<CasinoSectionGameDocument>,
            ...
            return codes
                .map(code => {
                    ...
                    const iconPath = iconNoExt.includes('/')
                        ? iconNoExt.split('/').map((s: string) => encodeURIComponent(s)).join('/')
                        : iconNoExt ? `${encodeURIComponent(g.provider)}/${encodeURIComponent(iconNoExt)}` : '';
    ...
    private readonly PROVIDER_FOLDERS = [
        "100HP", "7 Mojo", "AWC", "Aman&Aura Casino", "BetRadar",
        "BollyGaming RNG", "BollyTech", "CreedRoomz", "Darwin", "Dream Casino",
        "Evolution", "Ezugi", "Galaxsys", "InOut", "JackTop", "Marbles",
        "Ncasino", "Onlyplay", "PragmaticPlay", "Qtech", "RTG (NGP)",
        "SmartSoft", "SpinLogic", "Spribe(Aviator)", "SuperSpade", "Supernowa",
        "TVBET", "XProGaming"
    ];
    ...
    async getProvidersHub(category?: string) {
        try {
            const matchStage: any = { provider: { $ne: null }, isActive: true, icon: { $exists: true, $nin: [null, ""] } };
            ...
            const dbProviders = await this.casinoProviderModel.find({ isActive: true }).sort({ priority: -1, name: 1 });
            ...
            return result.filter(p => p.count > 0 && p.provider.toLowerCase() !== 'igtech');
        } catch (error) {
    ...
    async getGamesByProviderHub(provider: string, category?: string, search?: string, page: number = 1, limit: number = 60, type?: string) {
        try {
            const query: any = { isActive: true, icon: { $exists: true, $nin: [null, ""] } };
            let sort: any = { playCount: -1, createdAt: -1 };

            // Start filter logic for type
            if (type === 'live') {
                const liveFilter = this.expandCategorySearch('live casino');
                Object.assign(query, liveFilter);
            }
            // End filter logic
            ...
            if (provider && provider !== 'all') {
                ...
                else searchTerm = provider.replace(/\(.*\)/, '').replace(/Live|Slot/gi, '').trim();

                query.provider = { $regex: searchTerm, $options: 'i' };
            }

            // ── Section shorthand keys — serve admin-pinned games ──────────
            const SECTION_KEYS = ['popular', 'new', 'slots', 'live', 'table', 'crash', 'home', 'top', 'exclusive', 'trending'];
            if (category && SECTION_KEYS.includes(category)) {
                ...
                if (category === 'popular') {
                    ...
                    const catFilter = this.expandCategorySearch(category);
            ...
            const mappedGames = games.map((g: any) => ({
                ...
                gameType: g.subType || g.type || g.category || 'SLOT',
                banner: (() => {
                    if (!g.icon) return g.image || `https://images.unsplash.com/photo-1605218427306-022ba8c15661?q=80&w=600&auto=format&fit=crop`;
            ...
            if (mappedGames.length > 0) {
                this.logger.debug(`[CF Image Debug] First game: ${mappedGames[0].gameName} | provider: ${mappedGames[0].providerCode} | banner: ${mappedGames[0].banner}`);
            }
    ...
    async getCategoriesHub(type?: 'live' | 'casino') {
        try {
            const popularCount = await this.casinoGameModel.countDocuments({ playCount: { $gt: 100 } });
            const newCount = 50;
            const totalGames = await this.casinoGameModel.countDocuments({ isActive: true });

            // Aggregate categories from CasinoGame
            const matchStage: any = { category: { $ne: null }, isActive: true, icon: { $exists: true, $nin: [null, ""] } };
            ...
            const categoryMap = new Map<string, { count: number; originalNames: Set<string> }>();
            ...
            const normalize = (cat: string) => {
                ...
                if (n === 'live' || n === 'live dealer' || n === 'live casino' || n === 'live popular' || n === 'live games') return 'live casino';
                ...
                if (n.includes('lucky') || n.includes('lotto') || n.includes('lottery')) return 'lottery';
                return n;
            };
            ...
            if (popularCount > 0) {
                sortedCategories.unshift({ id: 'popular', name: 'Popular', count: popularCount > 100 ? 100 : popularCount, originalNames: [] });
            }
            if (newCount > 0) {
    ...
    async getGameUrlHub(username: string, provider: string, gameId: string, isLobby: boolean = false, walletMode?: string) {
        try {
            ...
            const casinoUserBonus = !walletMode ? await (this.prisma as any).userBonus.findFirst({
                where: {
                    userId: user.id,
                    status: 'ACTIVE',
            ...
            if (resolvedMode === 'cryptobonus') {
                ...
                walletBalance = ((user as any).casinoBonus ?? 0) + ((user as any).fiatBonus ?? 0);
                walletCurrency = 'INR';
            ...
            const backendDomain = process.env.NEXT_PUBLIC_API_URL || 'https://zeero.bet/api';
            ...
            const walletSuffixCode = walletSuffix === '_main'        ? 'm'
                                   : walletSuffix === '_usd'         ? 'u'
                                   : walletSuffix === '_fiatbonus'   ? 'f'
                                   : walletSuffix === '_cryptobonus' ? 'c'
                                   : 'm';
            const memberAccount = `${this.HUIDU_PLAYER_PREFIX}${user.id}${walletSuffixCode}`.replace(/[^a-zA-Z0-9]/g, '');
            ...
            // Encrypt parameters
            const encryptedPayload = this.huiduCryptoService.encrypt(parametersObj, this.HUIDU_AES_KEY);
            ...
            this.logger.log(`[HUIDU Launch DEBUG] Requesting game ${gameUid} with agency_uid ${this.HUIDU_AGENCY_UID}`);
            ...
            if (data.code === 0 && data.payload && data.payload.game_launch_url) {
                ...
                throw new HttpException(data.msg || 'Failed to launch game (HUIDU rejected)', 400);
            }
        } catch (error) {
            this.logger.error(`getGameUrlHub error for user = ${username}, gameId = ${gameId}, provider = ${provider}: ${error.message}`);
            if (error.response?.data) {
                 this.logger.error(`HUIDU Detailed Response Error: ${JSON.stringify(error.response.data)}`);
            }
            throw new HttpException(
                error.response?.data?.msg || error.message || 'Internal casino launch error', 
    ...
    async huiduWalletCallbackHub(body: any) {
        try {
            ...
            const { member_account, game_uid, serial_number, bet_amount, win_amount } = data;
            ...
            const walletType: 'fiat' | 'crypto' = (detectedMode === 'usd' || detectedMode === 'cryptobonus') ? 'crypto' : 'fiat';
            ...
            const result = await this.prisma.$transaction(async (prisma) => {
                ...
                // 2. Check Idempotency based on serial_number
                const existingTx = await prisma.casinoTransaction.findFirst({
                    where: { txn_id: serial_number }
                });

                // If already processed, we just return the *current* balance successfully without deducting again
                if (existingTx) {
                    this.logger.log(`Webhook: Idempotent hit for bet ${serial_number}, returning current balance`);
                    return { balance: parseFloat(user.balance.toString()), alreadyProcessed: true };
                }
                ...
                if (detectedMode === 'cryptobonus') {
                    balanceField = 'cryptoBonus';
                    currentBalance = (user as any).cryptoBonus ?? 0;
                } else if (detectedMode === 'fiatbonus') {
                    balanceField = 'casinoBonus';
                    currentBalance = (user as any).casinoBonus ?? 0;
                } else if (detectedMode === 'usd') {
                    balanceField = 'cryptoBalance';
                    currentBalance = (user as any).cryptoBalance ?? 0;
                } else {
                    balanceField = 'balance';
                    currentBalance = parseFloat(user.balance.toString());
                }

                const newBalance = Math.max(0, currentBalance - betNum + winNum);

                if (currentBalance < betNum && betNum > 0 && winNum === 0) {
                    throw new Error('Insufficient balance');
                }

                // Update only the sub-wallet that was charged (no cross-wallet mixing)
                const updatedUser = await prisma.user.update({
                    where: { id: userId },
                    data: { [balanceField]: newBalance } as any
                });

                // Look up game name from MongoDB (best-effort, non-critical)
                let gameName: string | null = null;
                try {
                    const gameDoc = await this.casinoGameModel.findOne({ gameCode: game_uid }).select('name').lean();
                ...
                await prisma.casinoTransaction.create({
                    data: {
                        ...
                        type: winNum > betNum ? "WIN" : (betNum > 0 ? "BET" : "UPDATE"),
                        wallet_type: walletType,   // "fiat" | "crypto"
                        timestamp: new Date()
                    } as any
                });

                return { balance: newBalance, updatedUser, alreadyProcessed: false, activeWallet: walletType, detectedMode };
            });
            ...
            if (!result.alreadyProcessed && result.updatedUser) {
                ...
                if (wagerAmount > 0) {
                    this.bonusService.recordWagering(userId, wagerAmount, 'CASINO', detectedMode).catch(e => {
                        this.logger.error(`[BonusWagering] Casino failed for user ${userId}: ${e.message}`);
            ...
            const encryptedResponse = this.huiduCryptoService.encrypt(responseObj, this.HUIDU_AES_KEY);

            this.logger.log(`Processed Huidu Bet: user ${userId}, bet: ${bet_amount}, win: ${win_amount} -> new balance: ${result.balance} `);
    ...
    async updateGame(id: string, data: any) {
        return this.casinoGameModel.findByIdAndUpdate(id, data, { returnDocument: 'after' });
    }
    ...
    async syncGames() {
        ...
        this.huiduApiService.syncHuiduData().catch(e => this.logger.error('Manual sync failed', e));
    ...
    async getAdminCategories() {
        return this.casinoCategoryModel.find().sort({ priority: -1, name: 1 }).exec();
    }
    ...
    async updateCategory(id: string, data: any) {
        ...
        return this.casinoCategoryModel.findByIdAndUpdate(id, data, { returnDocument: 'after' });
    }
    ...
    async getAdminProviders() {
        return this.casinoProviderModel.find().sort({ priority: -1, name: 1 }).exec();
    }
    ...
    async updateProvider(id: string, data: any) {
        return this.casinoProviderModel.findByIdAndUpdate(id, data, { returnDocument: 'after' });
    }
    ...
    // Games
    async getAdminAllGames(page: number = 1, limit: number = 20, search?: string, provider?: string, category?: string) {
        ...
        const games = await this.casinoGameModel.find(query)
    ...
    async deleteGame(id: string) {
        // Soft delete
        return this.casinoGameModel.findByIdAndUpdate(id, { isActive: false }, { returnDocument: 'after' });
    }
}
